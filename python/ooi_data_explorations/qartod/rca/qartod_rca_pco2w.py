...
@brief Load the PCO2W data from the RCA fixed platforms and processes
    the data to generate QARTOD Gross Range and Climatology test limits.
    Code revised from *qartod_ce_pco2w.py* to work with RCA streamed
...
from ooi_data_explorations.cabled.process_pco2w import pco2w_streamed, quality_checks
...
def generate_qartod(site, node, sensor, cut_off, platform):
    ...
    """
    # load the streamed instrument data
    if 'PCO2WA' in sensor:
        dataRecord = 'pco2w_a_sami_data_record'
    elif 'PCO2WB' in sensor:
        dataRecord = 'pco2w_b_sami_data_record'

    data = load_gc_thredds(site, node, sensor, 'streamed', dataRecord, '^(?!.*cal).*PCO2W.*nc$')
    data = pco2w_streamed(data)
    ...
    gr_lookup = process_gross_range(data, ['pco2_seawater'], [200, 2000], site=site, node=node, sensor=sensor, stream=dataRecord, source=gr_source)

    # create and format the climatology entry and table
    if 'fixed' in platform:
        clm_lookup, clm_table = process_climatology(data, ['pco2_seawater'], [200, 2000], site=site, node=node, sensor=sensor, stream=dataRecord)
        ...
        ## pCO2 is taken only on the stepped downcast twice a day...climatology bins must match the stepped pressure stops for the downcast
        ...
        for pressBin in bins:
            ...
            clm_lookup, clm_table = process_climatology(data_bin, ['pco2_seawater'], [200, 2000], site=site, node=node, sensor=sensor, stream=dataRecord)
        ...
        clm_lookup['parameters'][0] = {'inp': 'pco2_seawater', 'tinp': 'time', 'zinp': 'int_ctd_pressure'}
